...
int findOptimalPage(int *frame, int frame_size, int *ref_string, int n, int current_index)
{
    ...
    for (int i = 0; i < frame_size; i++)
    {
        ...
        // If page is never referenced again, replace it immediately
        if (j == n) 
        {
            return i;
        }

        // If all pages are found in the future, use the one farthest in the future
        if (replace_index == -1) 
        {
            replace_index = i;
        }
...
int main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        // If page is not found in the frame (i.e., page fault)
        if (!found)
        {
            int empty_found = 0;
